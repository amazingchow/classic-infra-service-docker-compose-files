...
case "$1" in
  ...
  'master')
  	ARGS="-volumePreallocate -volumeSizeLimitMB=1024"
  ...
  'server')
  	ARGS="-volume.max=0 -master.volumePreallocate -master.volumeSizeLimitMB=1024"
  	if [[ $@ == *"-volume.max="* ]]; then
  	  ARGS="-master.volumePreallocate -master.volumeSizeLimitMB=1024"
  	fi
  	exec /usr/bin/weed $@ $ARGS
  	;;
